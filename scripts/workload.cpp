...
#include "../Database/Txn_manager.h"
	...
	const string lubm10M_name = "lubm10M";
	const string dir = lubm10M_name + to_string(oltp_update_num) + '.' + to_string(oltp_query_num) ;
	...
	std::atomic<int> restart_times = {0};
	string lubm_q[21] = {
	...
	struct task1
	{
	    ...
	    bool is_empty()
	    {
		return updates.size() <= 0 && query.size() <= 0;
	    }
	};
	...
	task1 get_task_1(Txn_manager& txn_m, int k)
	{
	    task1 ret;
	    for(int i = 0; i < read_num; i++)
	    {
		tes++;
		ret.add_query(lubm_q[12/*tes % 21*/]);
	    }
	...
	task1 get_task(Txn_manager& txn_m)
	{
	    task1 ret;
	    for(int i = 0; i < read_num; i++)
	    {
	...
	void P(std::atomic<int> a[], char* str)
	{
	    int sm = 0;
	    cerr << str << " : ";
...
//    for(int i = 1; i <= threads_num; i++)cerr << IVEntry_ex[i] << " ";
    P(IVEntry_ex, "IVEntry_ex");
    P(IVEntry_sh, "IVEntry_sh");   
    
    P(IVArray_ex, "IVArray_ex");
    P(IVArray_sh, "IVArray_sh");

//    P(Database_insert, "Database_insert");
//    P(Database_insertT, "Database_insertTriple");

//    P(StringIndex1, "StringIndex_change");

//    P(DB_query, "DB_query");

    P(GVBK, "GetValuebyKey");

//    cerr << "insertTriple average time: " << ((Database_insertT[0])?(Time1/Database_insertT[0]):-1) << endl;
//    cerr << "StringIndex_change average time: " << ((StringIndex1[0])?(Time2/StringIndex1[0]):-1) << endl;
