...
CFLAGS = -c -Wall -pthread -ltbb -std=c++11 -g3 -gdwarf-2	# For debugging
# EXEFLAG = -O2 -pthread -std=c++11
#-coverage
# CFLAGS = -c -Wall -pthread -g -std=c++11 -pg
EXEFLAG = -pthread -g3 -std=c++11
...
library = -lreadline -L./lib -L/usr/local/lib -L/usr/lib/ -lantlr4-runtime -lgcov -lboost_thread -lboost_filesystem -lboost_system -lboost_regex -lpthread -ltbb -I/usr/local/include/boost -lcurl -latomic
...
TARGET = $(exedir)gexport $(exedir)gbuild $(exedir)gserver $(exedir)gserver_backup_scheduler $(exedir)gclient $(exedir)gquery $(exedir)gconsole $(api_java) $(exedir)gadd $(exedir)gsub $(exedir)ghttp $(exedir)gmonitor $(exedir)gshow $(exedir)shutdown $(exedir)ginit $(exedir)gdrop $(testdir)update_test $(testdir)dataset_test $(testdir)transaction_test $(testdir)run_transaction $(testdir)lubm_query $(testdir)workload $(exedir)gbackup $(exedir)grestore $(exedir)gpara $(exedir)rollback 
...
$(testdir)lubm_query: $(lib_antlr) $(objdir)lubm_query.o $(objfile)
	$(CC) $(EXEFLAG) -o $(testdir)lubm_query $(objdir)lubm_query.o $(objfile) $(library) $(openmp)
#executables end
...
$(objdir)lubm_query.o: $(testdir)lubm_query.cpp Database/Database.h Util/Util.h $(lib_antlr)
	$(CC) $(CFLAGS) $(testdir)lubm_query.cpp $(inc) -o $(objdir)lubm_query.o $(openmp)
...
$(objdir)TempResult.o: Query/TempResult.cpp Query/TempResult.h Query/RegexExpression.h $(objdir)Util.o \
	$(objdir)StringIndex.o $(objdir)KVstore.o $(objdir)QueryTree.o $(objdir)Varset.o $(objdir)EvalMultitypeValue.o
...
$(objdir)StringIndex.o: StringIndex/StringIndex.cpp StringIndex/StringIndex.h $(objdir)KVstore.o $(objdir)Util.o  $(objdir)KVstore.o
...
clean:
	...
	rm -rf bin/*.class
	rm -rf $(testdir)update_test $(testdir)dataset_test $(testdir)transaction_test $(testdir)run_transaction $(testdir)workload $(testdir)lubm_query
...
test0: $(TARGET)
	rm -rf lubm1M.db
	cp -r ../lubm1M.db .
	./scripts/workload > ./massage.txt

test1: $(TARGET)
	rm -rf lubm10M.db
	cp -r ../gStore_origin/lubm10M.db .
	./scripts/workload > /dev/null
