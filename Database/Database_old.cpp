...
std::atomic<int> Database_insert[11], Database_insertT[11];
std::atomic<int> IR, IRT;
std::atomic<int> Time1, Time2;
...
std::atomic<int> DB_query[11];
std::atomic<int> dbquery;
...
Database::query(const string _query, ResultSet& _result_set, FILE* _fp, bool update_flag, bool export_flag, shared_ptr<Transaction> txn)
{
	...
	if(!export_flag)
	{
		...
		// 	//if(ans_num != 0) cerr << "ans_num is not zero! " << endl; 
...
Database::insertTriple(const TripleWithObjType& _triple, vector<unsigned>* _vertices, vector<unsigned>* _predicates, shared_ptr<Transaction> txn)
{
	...
	Database_insertT[IRT.load()]++;
//	if(IRT > 1)
//		cerr << "insertTriple: " << IRT << endl;
...
//	this->stringindex->change(predicates, *this->kvstore, false);
	...
	ull ti3 = Util::get_cur_time();

	IR--; 

	Time1.fetch_add(ti2-ti1);
	Time2.fetch_add(ti3-ti2);

//	cerr << "intT time: " << ti2-ti1 << "  and change time: " << ti3-ti2 << endl;
