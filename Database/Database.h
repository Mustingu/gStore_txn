...
public:
	...
	std::mutex& get_query_parse_lock();
	...
	std::atomic<int> check_times = {0};
private:
	string name;
	string store_path;
	bool is_active;
	std::atomic<TYPE_TRIPLE_NUM> triples_num;
	TYPE_ENTITY_LITERAL_ID entity_num;
	std::atomic<TYPE_ENTITY_LITERAL_ID> sub_num;
	...
	// for allocEntityID
	std::mutex allocEntityID_lock;
	//for allocLiteralID
	std::mutex allocLiteralID_lock;
	//for allocPredicateID
	std::mutex allocPredicateID_lock;
	//for log file
	std::mutex log_lock;
