...
{
	//cerr << "InIn getpreIDlistBysubID " << _subid << endl;
...
{
	//cerr << "InIn getobjIDlistBysubID " << _subid << endl;
	if (!Util::is_entity_ele(_subid)) {
		_objidlist = NULL;
		_list_len = 0;
		return false;
...
{
	//cerr << "InIn getobjIDlistBysubIDpreID " << _subid << ' ' << _preid << endl;
	if (!Util::is_entity_ele(_subid)) {
		_objidlist = NULL;
		_list_len = 0;
		return false;
...
{
	//cerr << "InIn getpreIDobjIDlistBysubID " << _subid << endl;
	if (!Util::is_entity_ele(_subid)) 
	{
...
KVstore::getsubIDlistBypreID(TYPE_PREDICATE_ID _preid, unsigned*& _subidlist, unsigned& _list_len, bool _no_duplicate, shared_ptr<Transaction> txn) const 
{
	...
	//cerr << "InIn getsubIDlistBypreID " << _preid << endl;
	unsigned* _tmp = NULL;
	unsigned long _len = 0;
	if(txn == nullptr)
	{
...
KVstore::getobjIDlistBypreID(TYPE_PREDICATE_ID _preid, unsigned*& _objidlist, unsigned& _list_len, bool _no_duplicate, shared_ptr<Transaction> txn) const {
	//cerr << "InIn getobjIDlistBypreID " << _preid << endl;
	unsigned* _tmp = NULL;
	unsigned long _len = 0;
...
KVstore::getsubIDobjIDlistBypreID(TYPE_PREDICATE_ID _preid, unsigned*& _subid_objidlist, unsigned& _list_len, bool _no_duplicate, shared_ptr<Transaction> txn) const 
{
	...
	//cerr << "InIn getsubIDobjIDlistBypreID " << _preid << endl;
#ifdef DEBUG_KVSTORE
...
KVstore::getpreIDlistBysubIDobjID(TYPE_ENTITY_LITERAL_ID _subid, TYPE_ENTITY_LITERAL_ID _objid, unsigned*& _preidlist, unsigned& _list_len, bool _no_duplicate, shared_ptr<Transaction> txn) const 
{
	...
	//cerr << "InIn getpreIDlistBysubIDobjID " << _subid << ' ' << _objid << endl;
...
KVstore::binarySearch(unsigned _key, const unsigned* _list, unsigned _list_len, int _step) 
{
	...
	while (_left <= _right) {
		...
		//if(_step * _mid >= _list_len) cerr << "error" << endl;
...
unsigned short KVstore::buffer_literal2id_query = 32;
unsigned short KVstore::buffer_id2literal_query = 32;
...
unsigned short KVstore::buffer_sID2values_query = 256;
unsigned short KVstore::buffer_oID2values_query = 256;
unsigned short KVstore::buffer_pID2values_query = 256;

//MVCC 

//MVCC

std::atomic<int>KV_sh;
...
//	KV_sh++;
//	if(KV_sh > 1)cerr << "YES" << KV_sh << endl;
	//cout << "getValueByKey                  " << _key << FirstRead << endl;
	//cout << "this is transaction getValueByKey ..................." << endl;
	if (Util::is_literal_ele(_key) && _array == objID2values)
	{
		unsigned key = _key - Util::LITERAL_FIRST_ID;
		bool ret = objID2values_literal->search(key, _val, _vlen, AddSet, DelSet, txn, latched, FirstRead);
//		KV_sh--;
		return ret;
	}
	bool ret = _array->search(_key, _val, _vlen, AddSet, DelSet, txn, FirstRead);
//	KV_sh--;
	return ret;
}
...
{
	//cerr << "this is Insert_values................: " << _key <<  endl;
	if (Util::is_literal_ele(_key) && _array == objID2values)
	{
...
{
//	if(KVs>1)
//		cerr << "get exlatch from kvstore:" << KVs << endl;
