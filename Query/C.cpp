	...
	limitID_entity(_limitID_entity), temp_result(NULL), fp(NULL), export_flag(false), csr(_csr), txn(_txn)
{
	...
	//forbidden query cache here
	if(_txn != nullptr){
		this->query_cache = nullptr;
	}
}
						...
						if (isel[k])
						{
							for (unsigned i = 0; i < retAnsNum; i++)
							{
								...
								if (ans_id != INVALID)
								{
									if (ans_id < Util::LITERAL_FIRST_ID)
										//a[0]->addRequest
									    t[i*selectVar + j] = kvstore->getStringByID(ans_id);
						...
						else
						{
							for (unsigned i = 0; i < retAnsNum; i++)
							{
								...
								if (ans_id != INVALID){
                                  ...
                                  t[i*selectVar + j] =  kvstore->getPredicateByID(ans_id);
								}
							}
						}
					}
				...
				for (int j = 0; j < ret_result.select_var_num; j++)
				{
					...
					if (k != -1)
					{
						if (k < id_cols)
						{
							...
							{
							  if(isel[k])
                                ans_str = kvstore->getStringByID(ans_id);
							  else
                                ans_str = kvstore->getPredicateByID(ans_id);
