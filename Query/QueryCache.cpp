...
{
	lock_guard<std::mutex> lck(this->query_cache_lock);  //when quit this scope the lock will be released
...
{
	lock_guard<std::mutex> lck(this->query_cache_lock);  //when quit this scope the 
	//query_cache_lock.lock();
	time_now = 0;
	total_memory_used = 0;
